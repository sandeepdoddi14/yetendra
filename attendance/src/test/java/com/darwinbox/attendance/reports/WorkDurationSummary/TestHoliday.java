    ...
    public void testWorkDuration(Map<String, String> testData) throws Exception {
        ...
        for (int j = 1; j <= 6; j++) {
            ...
            if (isPresent) {
                ...
                log.info("check->"+attendanceImportPage.getBreakDuration()+"--"+attendanceImportPage.getInTime()+"--"+attendanceImportPage.getOutTime());
                long start = dateTimeHelper.parseTime(dateTimeHelper.parseTime(shift.getStartTime()));
                long end = dateTimeHelper.parseTime(dateTimeHelper.parseTime(shift.getEndTime()));
                long totalDur = end - start;
                log.info(totalDur+"check");
        ...
        for (AttendanceSettingsPage.DAYS day : AttendanceSettingsPage.DAYS.values()) {
            ...
            Reporter("Work Duration is 0","INFO");
