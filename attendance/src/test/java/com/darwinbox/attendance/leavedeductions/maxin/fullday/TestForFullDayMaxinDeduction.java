package com.darwinbox.attendance.leavedeductions.maxin.fullday;
...
public class TestForFullDayMaxinDeduction extends TestBase {
    ...
    public void testBasicFullDayDeduction(Map<String, String> testData) {
        ...
        for (LeaveDeductionsBase.DAYSTATUS day : LeaveDeductionsBase.DAYSTATUS.values()) {
            ...
            try {
                ...
                boolean proceed = ((isholiday && lateDuration.isHoliday()) ||
                        ( isWeekoff && lateDuration.isWeekoff() && (!isholiday) ) || empty ) ;

                if ( proceed ) {
                    if (lateDuration.isApprovalRequired())
                        Assert.assertTrue(status.contains(atb.pending + "(" + leaveName + ")"), "No Leave is Deducted and in Pending state");
                    else
                        Assert.assertTrue(status.contains(atb.approved + "(" + leaveName + ")"), "No Leave is Deducted and Approved");
                }
