package com.darwinbox.attendance.leavedeductions.lateearly.halfday;
...
public class TestSecondHalfAppliedAndApprovedForLateEarlyHalfDayDeduction extends TestBase {
    ...
    public void testSecondHalfAppliedAndApprovedForLateEarlyHalfDayDeduction(Map<String, String> testData) {
        ...
        for (LeaveDeductionsBase.DAYSTATUS day : LeaveDeductionsBase.DAYSTATUS.values()) {
            ...
            for ( Date d : dates ) {
                ...
                boolean proceed = latePlusEarly.getProceed(latePlusEarly, day) && ( count >= latePlusEarly.getCount() );

                if (proceed) {
                    atb.validateLeave(!latePlusEarly.isApprovalRequired(), isFirst || isSecond, status, leaveName, this);
                } else {
