package com.darwinbox.attendance.leavedeductions.finalduration.halfday;
...
public class TestFirstHalfAppliedAndApprovedForFinalDurationHalfDayDeduction extends TestBase {
    ...
    @Test(dataProvider = "TestRuns", dataProviderClass = TestDataProvider.class, groups = "FinalDuration,LeaveDeduction", retryAnalyzer = TestBase.class)
    public void testFirstHalfAppliedAndApprovedForFinalDurationHalfDayDeduction(Map<String, String> testData) {
        ...
        if (workDuration == null) {
            Reporter("FinalDuration deduction is disabled ", "FAIL");
            return;
        }
        ...
        for (LeaveDeductionsBase.DAYSTATUS day : LeaveDeductionsBase.DAYSTATUS.values()) {
            ...
            if (proceed) {
                atb.validateLeave(!workDuration.isApprovalRequired(), true, status, leaveName, this);
            } else {
