package com.darwinbox.leaves.Accural.CarryForwardTests.CarryForwardWithTenure.TwoCyclesWithEncashment;
...
public class August extends LeaveAccuralBase {
    ...
    public void verifyCFWithTenure(Map<String, String> testData) throws  Exception {
        ...
        response=new LeaveAdmin().applyEncashmentWithEmpSession(employee,serverChangedDate,noOfLeaves+"",new LeaveService().getLeaveID(carryForwardBalance.getLeave_Type(),carryForwardBalance.groupCompanyMongoId));
        ...
        Reporter("No of Leaves Encashed -->" +noOfLeaves,"Info");
        ...
        carryForwardBalance.setMaximum_leave_allowed_per_year(carryForwardBalance.getMaximum_leave_allowed_per_year()-noOfLeaves);
        super.setLeavePolicyObject(carryForwardBalance);
        ...
        if(response.contains("failure"))
        {
         throw new Error("Error in applying Leave --> "+response);
        }
        Reporter("No of Leaves Encashed -->" +noOfLeaves,"Info");
        ...
        if(!leavePolicyObject.getCredit_on_pro_rata_basis().indicator)
        {
           monthDiff= (int) new DateTimeHelper().getMonthDifferenceBetweenTwoDates(leaveCycleStartDate.toString(),firstAnniverseryDate.toString());
            untillFirstAniversery = (untillFirstAniversery/12)*monthDiff-noOfLeaves;
        }

        carryForwardBalance.setMaximum_leave_allowed_per_year(Integer.parseInt(testData.get("Credit No of Leaves").split(",")[0]));

        changeServerDate(firstAnniverseryDate.plusDays(1));
        ...
        carryForwardBalance.setMaximum_leave_allowed_per_year(carryForwardBalance.getMaximum_leave_allowed_per_year());
        super.setLeavePolicyObject(carryForwardBalance);
        ...
        if(!leavePolicyObject.getCredit_on_pro_rata_basis().indicator)
        {
            ...
            afterFirstAnniverseryBalance = (afterFirstAnniverseryBalance/12)*monthDiff-noOfLeaves;
        }
