package com.darwinbox.leaves.ProductionIssues.DBX_7691.CarryForwardWithLeaveEncashmentAndLeaves_S2;
...
public class September extends LeaveAccuralBase {
    ...
    public void VerifyCarryForwardWithLeaveEncashmentAndLeaves(Map<String, String> testData) throws  Exception{
        ...
        int noOfLeaves =2;
        leaveCycleStartDate = LocalDate.parse("2019-09-01");
        leaveCycleEndDate = LocalDate.parse("2020-08-31");
        ...
        changeServerDate(LocalDate.now());
            ...
            try {
                  employee = (new EmployeeServices().generateAnEmployee("no", "Working Days (DO NOT TOUCH)", leaveCycleStartDate.minusMonths(1).toString(), "no"));
            } catch (Exception e) {
                try {
                    employee = (new EmployeeServices().generateAnEmployee("no", "Working Days (DO NOT TOUCH)", leaveCycleStartDate.minusMonths(1).toString(), "no"));
                } catch (Exception e1) {
           ...
           Reporter("Employee DOJ is --> "+employee.getDoj(),"INFO");
             ...
             changeServerDate(LocalDate.parse(employee.getDoj()));
            ...
            logoutFromSession();
               // Thread.sleep(5000);
                ...
                changeServerDate(LocalDate.parse(employee.getDoj()).plusDays(forwardDateForEncashmnet));
        ...
        forwardDateForEncashmnet=leavePage.workingDays.length;
        changeServerDate(LocalDate.parse(serverChangedDate).plusDays(forwardDateForEncashmnet));
        ...
        loginpage.switchToAdmin();
         ...
         messageId = new LeaveAdmin().getMessageId(employee);
        new LeaveAdmin().encashmentAction(messageId, "accept");
        ...
        expecetedLeaveBalance = expecetedLeaveBalance  + calculateLeaveBalance(leaveCycleStartDate.toString(), leaveCycleEndDate.toString())-noOfLeaves*2;
        Reporter("Expected CF Leave Balance is --" + expecetedLeaveBalance, "Info");
        ...
        expecetedLeaveBalance = expecetedLeaveBalance  +leavePolicyObject.getMaximum_leave_allowed_per_year()-noOfLeaves*2;
        Reporter("Expected Total Leave Balance is --" + expecetedLeaveBalance, "Info");


        actualLeaveBalance = new LeaveBalanceAPI(employee.getEmployeeID(), carryForwardBalance.getLeave_Type()).getBalance();
        Reporter("Actual Total Leave Balance is ---" + actualLeaveBalance, "Info");
