...
public class Verify_WorkingDays_Tenure_Deactivation_Leave_Balance extends TestBase{
...
private static final Logger log = Logger.getLogger(Verify_WorkingDays_Tenure_Deactivation_Leave_Balance.class);
...
public void Verify_Deactivation_Leave_Balance_of_an_employee_for_all_scenarios_without_creating_Leaves(Map<String,String> data) throws Exception {

		String[] deactivationDates = {DateTimeHelper.getCurrentLocalDate()};
		...
		if((i==0 || i%25 == 0) && UtilityHelper.getProperty("config", "Server.Date.Change.Required").equalsIgnoreCase("Yes")) {
		...
		Assert.assertTrue(leavesAction.verifyTenureDeactivationLeaveBalanceForFourEdgeDaysCustomDatesNew(deactivationDates), "WDWithDeactivation Leave Balance for whole leave cycle calculated successfully") ;
