...
public class TC_08_Verify_Admin_is_able_to_edit_policy extends TestBase {
	...
	public void Verify_Admin_is_able_to_edit_policy(Map<String,String> data) throws Exception {
		...
		Assert.assertTrue(attPolicySettings.clickPoliciesLink(), "Click on Policy link");
			...
			Assert.assertTrue(attPolicySettings.selectLeaveDeductionPolicy(data.get("Leave Deduction Policy_Dropdown")),"Select Leave deduction policy");
			Assert.assertTrue(attPolicySettings.enableOrDisableLateMarkPolicy(data.get("Late Mark Policy_Dropdown")),"Select Late Mark policy");
			Assert.assertTrue(attPolicySettings.enableOrDisableWorkDurationPolicy(data.get("Work Duration Policy_Dropdown")),"Select Work Duration policy");
			Assert.assertTrue(attPolicySettings.enableOrDisableEarlyMarkPolicy(data.get("Early Mark Policy_Dropdown")),"Select Early Mark Duration policy");
			...
			Assert.assertTrue(attPolicySettings.insertPolicyName(data.get("Policy Name_TextBox")+ "_Check_Edit"),"Insert Policy Name.");
			Assert.assertTrue(attPolicySettings.clickSaveButton(),"Save button clicked successfully");
